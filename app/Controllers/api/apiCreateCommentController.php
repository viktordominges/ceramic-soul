...
function apiCreateCommentController() {
    try {
        ...
        // Простая валидация
        if (empty($text)) {
            return json_response(['error' => 'Комментарий не может быть пустым'], 400);
        }

        if ($post_id <= 0) {
            return json_response(['error' => 'Некорректный ID поста'], 400);
        }
        ...
        if ($success) {
            return json_response(['message' => 'Комментарий успешно добавлен'], 201);
        }

        return json_response(['error' => 'Не удалось создать комментарий'], 500);
